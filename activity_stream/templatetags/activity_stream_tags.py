...
def users_activity_stream(user, count):
    activity_items = ActivityStreamItem.objects.filter(actor=user, subjects__isnull=False, created_at__lte=datetime.datetime.now()).order_by('-created_at').distinct()
    return {"activity_items": activity_items, "count": count}
...
def following_activity_stream(user, count):
    ...
    list(following).append(user)   
    activity_items = ActivityStreamItem.objects.filter(actor__in=following, subjects__isnull=False, created_at__lte=datetime.datetime.now()).order_by('-created_at').distinct()
    return {"activity_items": activity_items, "count": count}


@register.inclusion_tag("activity_stream/global_activity_stream.html")
def global_activity_stream(count, privacylevel=0):
    activity_items = ActivityStreamItem.objects.filter(subjects__isnull=False, created_at__lte=datetime.datetime.now()).order_by('-created_at').distinct()
    return {"activity_items": activity_items, "count": count}
